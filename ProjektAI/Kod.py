...
def save_current_fig(title):
    ...
    # Pobierz aktualną figurę i ustaw dużą rozdzielczość
    fig = plt.gcf()
    fig.set_size_inches(20, 12)  # pełny ekran, proporcje 16:9
    plt.savefig(filename, bbox_inches='tight', dpi=300)  # wysoka jakość
    logging.info(f"Zapisano grafikę: {filename}")

# --- Funkcje wizualizacji i metryk ---

def plot_prototypes(model, X, y, title):
    logging.info(f"Rysowanie prototypów dla: {title}")
    pca = PCA(2)
...
def plot_param_dependency_3d(model_class, param_grid, fixed_params, X, y,
    ...
    for i, x_val in enumerate(xs):
        for j, y_val in enumerate(ys):
            ...
            Z[j,i] = accuracy_score(X, y_pred) if False else accuracy_score(y, y_pred)
...
def evaluate_gtol_dependency_for_all(models, gtol_values, fixed_params, X, y,
    ...
    for name, model_class in models.items():
        ...
        for g in gtol_values:
            ...
            model.fit(X, y)
            y_pred = model.predict(X)
            score = accuracy_score(y, y_pred) if metric=='accuracy' else None
...
for name, cls in models.items():
    ...
    logging.info(f"Tworzenie wykresu 3D dla {name}...")
    best = plot_param_dependency_3d(
