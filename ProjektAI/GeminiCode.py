...
def reset_parameters():
    """Resets global parameters to their initial values.
...
def train_and_evaluate_cv(model_class, name, X, y, cv=CV_FOLDS, params=None):
    if params is None:
...
def plot_maxiter_randomstate_dependency_3d_cv(model_class, max_iters, random_states, fixed_params, X, y, metric, title, cv=CV_FOLDS):
    ...
    for max_iter_val in max_iters:
        for random_state_val in random_states:
            params = fixed_params.copy()
            params['max_iter'] = max_iter_val
            params['random_state'] = random_state_val
            model = model_class(**params)
            ...
            try:
                ...
                logging.warning(f"Błąd dla max_iter={max_iter_val}, random_state={random_state_val}: {e}")
                mean_score = np.nan

            results.append((max_iter_val, random_state_val, mean_score))

    # Przekształcanie do siatki
    max_iter_vals_unique = sorted(set(r[0] for r in results))
    random_state_vals_unique = sorted(set(r[1] for r in results))
    Xg, Yg = np.meshgrid(max_iter_vals_unique, random_state_vals_unique)
    Z = np.empty_like(Xg, dtype=float)

    # Populate Z with scores
    for i, rs in enumerate(random_state_vals_unique):
        for j, mi in enumerate(max_iter_vals_unique):
            for r in results:
    ...
    if not valid_results:
        logging.error("No valid results for plotting. Skipping 3D plot.")
        return None
    ...
    ax.scatter(best[0], best[1], best[2], color='red', s=100, label='Najlepszy punkt')
    ax.scatter(worst[0], worst[1], worst[2], color='black', s=100, label='Najgorszy punkt')
...
for name, cls in models.items():
    train_and_evaluate_cv(cls, name, X_scaled, y, cv=CV_FOLDS, params=BASE_PARAMS)
